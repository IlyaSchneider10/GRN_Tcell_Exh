...
#edges.df <- mutate(frequency_interactions_plot, from=TF, to=target)
vertices.df <- data.frame(genes=unique(c(frequency_interactions_plot$target, frequency_interactions_plot$TF)))
...
for (x in names(regulon_targets)){
  regulon_targets[[x]]<-unique(unlist(regulon_targets[[x]]))
}
