...
for (x in names(regulons)){
  regulon_tfs<-append(regulon_tfs,names(regulons[[x]]))
}
unique_regulon_tfs<-unique(regulon_tfs)

all_regulon_interactions_trial<-vector("list", length(unique_regulon_tfs))
names(all_regulon_interactions_trial)<-sort(unique_regulon_tfs)

for (x in names(table(regulon_tfs))){
  all_regulon_interactions_trial[[x]]<-vector("list", length=table(regulon_tfs)[x])
}
