...
input_for_single_aucell<-input_for_single_aucell[frequncy_regulons_targets[frequncy_regulons_targets %in% row.names(input_for_single_aucell)],]

for (x in names(cell_types)){
  cell_types[[x]]<-input_for_single_aucell[,cell_types[[x]]$CELLID]
}

cell_types_scaled<-cell_types
for(x in names(cell_types)){
  cell_types_scaled[[x]]<-t(scale(t(cell_types[[x]])))
}

cell_types_scaled_avrg<-lapply(cell_types_scaled,rowMeans)
cell_types_avrg<-as.data.frame(cell_types_scaled_avrg)

heatmap(as.matrix(cell_types_avrg[1:250,]),Colv = NA, Rowv = NA, scale="none",col = bluered(100))
```
...
for (x in names(aucell_files)){
  aucell_files[[x]]<-aucell_files[[x]][,colnames(aucell_files[[x]]) %in% frequency_summary_nodes]
}
...
cell_types<-lapply(cell_types,scale)
cell_types<-lapply(cell_types,colMeans)
single_average_aucell<-as.data.frame(cell_types)

pdf(paste0(analysis_directory,'/figures/heatmap.pdf'),
     width=15,
     height=40)
