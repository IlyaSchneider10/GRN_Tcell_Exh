...
#Network of top 30 TFs from random forest 
```{r}
#Open frequency_summary
frequency_summary<-read.csv(paste0(analysis_directory,"/frequency_summary.csv"))
...
dev.off()
```
#Repeat two types of binarizings for 100 TFs from random forest
```{r}
...
#Network of top 100 TFs from random forest 
```{r}
...
#Remove STAT3 and NR2C2 since they are disconnected from the main graph
remove<-which(frequency_summary_top100$TF %in% c("STAT3","NR2C2"))
frequency_summary_top100<-frequency_summary_top100[-remove,]
...
plot(g, 
     layout=l,
     edge.arrow.size=0.1,
     edge.color=c("slategrey","dark red")[as.numeric(as.factor(E(g)$cistrome_filter))],
     vertex.size=(V(g)$degree/max(V(g)$degree)*25),
     vertex.label.cex=(V(g)$degree/max(V(g)$degree)*1.3),
...
pdf(paste0(analysis_directory,'/figures/frequency_interactions_top100_plot.pdf'),
     width=15,
     height=15)
plot(g, 
     layout=l,
...
#Louvain cluster our igraph
```{r}
louvain_clustering_res_0.7<-cluster_louvain(as.undirected(g),weight=frequency_summary_top100$NIS, resolution = 0.7)

#Get the communities
louvain_clustering_res_0.7_communities<-louvain_clustering_res_0.7$membership
names(louvain_clustering_res_0.7_communities)<-louvain_clustering_res_0.7$names
louvain_clustering_res_0.7_communities<-split(names(louvain_clustering_res_0.7_communities),louvain_clustering_res_0.7_communities)

#Plot the communities
pdf(paste0(analysis_directory,'/figures/frequency_interactions_top100_communities_plot.pdf'),
     width=15,
     height=15)
plot(louvain_clustering_res_0.7,g, 
     layout=l,
     edge.arrow.size=0.25,
     edge.color=c("grey","dark red")[as.numeric(as.factor(E(g)$cistrome_filter))],
     vertex.size=5,
...
#Plot each community
for (x in names(louvain_clustering_res_0.7_communities)){
   
  frequency_summary_com<-frequency_summary[which(frequency_summary$TF %in% louvain_clustering_res_0.7_communities[[x]]),]
  frequency_summary_com<-frequency_summary_com[which(frequency_summary_com$target %in% louvain_clustering_res_0.7_communities[[x]]),]
  ...
  pdf(paste0(analysis_directory,"/figures/louvain_clustering_res_0.7_communities/community_",as.character(x) ,".pdf"),
       width=15,
       height=15)
  plot(g, 
       layout=l,
...
#Create a new network and new communities using 90% as scenic filter
```{r}
#Select only interactions that are among top 100 TFs
frequency_summary_top100<-frequency_summary[which(frequency_summary$TF %in% aucell_model_importance_top100),]
frequency_summary_top100<-frequency_summary_top100[which(frequency_summary_top100$target %in% aucell_model_importance_top100),]
...
for (x in names(louvain_clustering_res_0.7_communities)){
  ...
  pdf(paste0(analysis_directory,"/figures/louvain_clustering_res_0.7_communities_90/community_",as.character(x) ,".pdf"),
       width=15,
       height=15)
  plot(g, 
       layout=l,
...
for (x in names(louvain_clustering_res_0.7_communities)){
  for (y in louvain_clustering_res_0.7_communities[[x]]){
...
for (x in seq(nrow(cytoscape))){
  ...
  cytoscape[x,"target_group"]<-cytoscape_nodes[pos,"group"]
}
...
#Save the table
setwd(analysis_directory)
write.table(cytoscape, file='cytoscape.tsv', quote=FALSE, sep='\t', row.names = F)
```
