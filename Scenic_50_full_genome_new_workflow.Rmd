...
plot(g_paper_overlap, 
     layout=l,
     edge.arrow.size=0.25,
     edge.color=c("grey","dark red")[as.numeric(as.factor(E(g_paper_overlap)$cistrome_filter))],
     vertex.size=5,
     vertex.label.cex=0.4,
     vertex.color=my_color,
...
#Substitute multiple TFs/targets
bolouri_multiple_tf_pos<-grep(":",bolouri_paper_interactions$TF)
bolouri_multiple_tf<-bolouri_paper_interactions$TF[bolouri_multiple_tf_pos[!duplicated(bolouri_paper_interactions$TF[bolouri_multiple_tf_pos])]]

bolouri_multiple_target_pos<-grep(":",bolouri_paper_interactions$target)
bolouri_multiple_target<-bolouri_paper_interactions$target[bolouri_multiple_target_pos[!duplicated(bolouri_paper_interactions$target[bolouri_multiple_target_pos])]]

bolouri_multiple_tf==bolouri_multiple_target

bolouri_multiple<-bolouri_multiple_tf

remove_multiple_bolouri_paper_entry<-function(str){
  one<-sub(":.*", "", str)
  two<-str_match_all(str, ":\\s*(.*?)\\s*:")[[1]][,2]
  three<-sub(".*:", "", str)
  four<-gsub(paste0(":|",one,"|",two,"|",three,"|"),"",str)
  
  res<-c(one, two, three,four)
}

bolouri_multiple_sub<-remove_multiple_bolouri_paper_entry(bolouri_multiple)

new_one<-data.frame(rep(bolouri_multiple_sub,length(bolouri_multiple_tf_pos)),rep(bolouri_paper_interactions[bolouri_multiple_tf_pos,]$target,length(bolouri_multiple_sub)))
colnames(new_one)<-colnames(bolouri_paper_interactions)[1:2]
new_one["Interaction"]<-rep(bolouri_paper_interactions[bolouri_multiple_tf_pos,]$Interaction,length(bolouri_multiple_sub))

new_two_targets<-c()
for (x in bolouri_multiple_sub){
  r<-rep(x,length(bolouri_multiple_target_pos))
  new_two_targets<-append(new_two_targets,r)
}
new_two<-data.frame(rep(bolouri_paper_interactions[bolouri_multiple_target_pos,]$TF,length(bolouri_multiple_sub)),new_two_targets)
colnames(new_two)<-colnames(bolouri_paper_interactions)[1:2]
new_two["Interaction"]<-rep(bolouri_paper_interactions[bolouri_multiple_target_pos,]$Interaction,length(bolouri_multiple_sub))

bolouri_new_entries<-rbind(new_one,new_two)

bolouri_paper_interactions<-bolouri_paper_interactions[-c(bolouri_multiple_tf_pos,bolouri_multiple_target_pos),]
bolouri_paper_interactions<-rbind(bolouri_paper_interactions,bolouri_new_entries)
bolouri_paper_interactions<-bolouri_paper_interactions[order(bolouri_paper_interactions$TF),]
...
for (x in seq(nrow(bolouri_paper_interactions_overlap))){
  if(bolouri_paper_interactions_overlap$TF[x]==bolouri_paper_interactions_overlap$target[x]){
    remove<-append(remove,x)
  }
}

bolouri_paper_interactions_overlap<-bolouri_paper_interactions_overlap[-remove,]
...
bolouri_paper_interactions_overlap[44,3]<-"inhibits"
...
bolouri_paper_interactions_overlap<-bolouri_paper_interactions_overlap[-c(53,54,55),]
bolouri_paper_interactions_overlap<-bolouri_paper_interactions_overlap[order(bolouri_paper_interactions_overlap$TF),]
rownames(bolouri_paper_interactions_overlap)<-seq(nrow(bolouri_paper_interactions_overlap))
...
bolouri_RF_fusion["present_in_both"]<-F
bolouri_RF_fusion[common_interactions,"present_in_both"]<-T
bolouri_RF_fusion["RF"]<-T
bolouri_RF_fusion["paper"]<-F

tfs<-which(bolouri_paper_interactions_overlap$TF==cytoscape_paper_overlap$TF[common_interactions])
targets<-which(bolouri_paper_interactions_overlap$target==cytoscape_paper_overlap$target[common_interactions])
pos<-which(tfs %in% targets)

bolouri_paper_unique_interactions<-bolouri_paper_interactions_overlap[,c(1,2)]
bolouri_paper_unique_interactions<-bolouri_paper_unique_interactions[-tfs[pos],]
bolouri_paper_unique_interactions["present_in_both"]<-F
bolouri_paper_unique_interactions["RF"]<-F
bolouri_paper_unique_interactions["paper"]<-T
bolouri_paper_unique_interactions["NIS"]<-1
bolouri_paper_unique_interactions["cistrome_filter"]<-F
...
nodes_to_add<-unique(c(bolouri_paper_unique_interactions$TF,bolouri_paper_unique_interactions$target))[!unique(c(bolouri_paper_unique_interactions$TF,bolouri_paper_unique_interactions$target)) %in% vertices.df_fusion$nodes]
...
nodes_to_add_expr<-c("mem","mem","exh","mem","rec","rec","rec","mem","mem","mem","exh","exh","exh","exh","mem","mem","exh","mem","rec","mem","exh","exh","rec","rec","rec","rec")
vertices.df_fusion[new_rows,"most_expressed_in"]<-nodes_to_add_expr

bolouri_paper_unique_interactions["TF_most_expressed_in"]<-"none"
bolouri_paper_unique_interactions["target_most_expressed_in"]<-"none"

for (x in seq(nrow(bolouri_paper_unique_interactions))){
  pos<-which(vertices.df_fusion$nodes==bolouri_paper_unique_interactions$TF[x])
  bolouri_paper_unique_interactions[x,"TF_most_expressed_in"]<-vertices.df_fusion[pos,"most_expressed_in"]
  
  pos2<-which(vertices.df_fusion$nodes==bolouri_paper_unique_interactions$target[x])
  bolouri_paper_unique_interactions[x,"target_most_expressed_in"]<-vertices.df_fusion[pos2,"most_expressed_in"]
}

#Create igraph to represent the combined network
igraph_fusion<-rbind(bolouri_RF_fusion[,c(1,4,7)],bolouri_paper_unique_interactions[,c(1,2,6)])
...
#Color nodes according to their highest expression
colrs <- c("#FB8072","#80B1D3","#EDD94C","#BEBADA")
...
plot(g_fusion, 
     ...
     vertex.frame.color='white')
legend(x=-1, y=-0.5, c("Exhausted","Memory","Receptors","Transition"), pch=21,
       col="#777777", pt.bg=colrs, pt.cex=2, cex=1.5, bty="n", ncol=1)
dev.off()
...
#Fuse the two tables
bolouri_paper_unique_interactions["TF_group"]<-0
bolouri_paper_unique_interactions["target_group"]<-0

bolouri_RF_fusion<-rbind(bolouri_RF_fusion,bolouri_paper_unique_interactions)
...
#Make sure that the present in both interaction is marked correctly
bolouri_RF_fusion[which(bolouri_RF_fusion$present_in_both==T),]$paper<-T
...
#Bonesis Toy
```{r}
bonesis_toy_nodes<-read.csv(paste0(analysis_directory,"/cytoscape/Output/cytoscape_bolouri_RF_fusion_Bonesis_toy_nodes.csv"),header=T)[,c(1,2,3)]
bonesis_toy_edges<-read.csv(paste0(analysis_directory,"/cytoscape/Output/cytoscape_bolouri_RF_fusion_Bonesis_toy_edges.csv"),header=T)[,3]
...
for (x in seq(nrow(bonesis_toy_interactions))){
  tf<-which(bonesis_toy_interactions$TF[x]==bolouri_paper_interactions$TF)
  target<-which(bonesis_toy_interactions$target[x]==bolouri_paper_interactions$target)
  pos<-tf %in% target
  if(length(pos)!=0){
    importance<-append(importance,tf[pos])
  }
}

bolouri_paper_interactions[importance,]
```

#Validate interactions from bolouri_RF_fusion_curated network
```{r}
#Export the interactions
bolouri_RF_fusion_curated_edges<-read.csv(paste0(analysis_directory,"/cytoscape/Output/cytoscape_bolouri_RF_fusion_curated_edge_2.csv"),header=T)

bolouri_RF_fusion_curated_edges_tf<-sub(" .*", "", bolouri_RF_fusion_curated_edges[,3])
bolouri_RF_fusion_curated_edges_target<-sub(".* ", "", bolouri_RF_fusion_curated_edges[,3])

bolouri_RF_fusion_curated_edges_tf<-gsub("CD3,","CD3, TCR, CD8, CD28",bolouri_RF_fusion_curated_edges_tf)
bolouri_RF_fusion_curated_edges_target<-gsub("CD28","CD3, TCR, CD8, CD28",bolouri_RF_fusion_curated_edges_target)

bolouri_RF_fusion_curated_edges["TF"]<-bolouri_RF_fusion_curated_edges_tf
bolouri_RF_fusion_curated_edges["target"]<-bolouri_RF_fusion_curated_edges_target

bolouri_RF_fusion_curated_edges<-bolouri_RF_fusion_curated_edges[,-c(2,3,8,9,10)]
bolouri_RF_fusion_curated_edges<-bolouri_RF_fusion_curated_edges[,c(6,7,1,2,3,4,5)]

bolouri_RF_fusion_curated_edges["cistrome_filter"]<-as.logical(bolouri_RF_fusion_curated_edges$cistrome_filter)
bolouri_RF_fusion_curated_edges["paper"]<-as.logical(bolouri_RF_fusion_curated_edges$paper)
bolouri_RF_fusion_curated_edges["present_in_both"]<-as.logical(bolouri_RF_fusion_curated_edges$present_in_both)
bolouri_RF_fusion_curated_edges["RF"]<-as.logical(bolouri_RF_fusion_curated_edges$RF)

#Select the ones from RF
bolouri_RF_fusion_curated_edges_RF<-bolouri_RF_fusion_curated_edges[which(bolouri_RF_fusion_curated_edges$RF==T),]
bolouri_RF_fusion_curated_edges_RF<-bolouri_RF_fusion_curated_edges_RF[,-c(5,6,7)]
bolouri_RF_fusion_curated_edges_RF<-bolouri_RF_fusion_curated_edges_RF[,c(1,2,4,3)]

#ATAC-seq validation
frequency_ataq_seq_summary<-frequency_ataq_seq_summary[which(frequency_ataq_seq_summary$frequency>=45),]

confirmed<-c()
for (x in seq(nrow(frequency_ataq_seq_summary))){
  tf<-which(bolouri_RF_fusion_curated_edges_RF$TF==frequency_ataq_seq_summary$TF[x])
  target<-which(bolouri_RF_fusion_curated_edges_RF$target==frequency_ataq_seq_summary$target[x])
  pos<-tf %in% target
  if(length(pos)!=0){
...
bolouri_RF_fusion_curated_edges_RF["atac_filter"]<-F
bolouri_RF_fusion_curated_edges_RF[confirmed,"atac_filter"]<-T

#Dorothea
confirmed<-c()
for (x in seq(nrow(dorothea_hs))){
  tf<-which(bolouri_RF_fusion_curated_edges_RF$TF==dorothea_hs$tf[x])
  target<-which(bolouri_RF_fusion_curated_edges_RF$target==dorothea_hs$target[x])
  pos<-tf %in% target
  if(length(pos)!=0){
    confirmed<-append(confirmed,tf[pos])
  }
...
bolouri_RF_fusion_curated_edges_RF["dorothea_filter"]<-F
bolouri_RF_fusion_curated_edges_RF[confirmed,"dorothea_filter"]<-T
bolouri_RF_fusion_curated_edges_RF[confirmed,]
