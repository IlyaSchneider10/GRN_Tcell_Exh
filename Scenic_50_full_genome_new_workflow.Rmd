...
#bonesis_toy_tfs<-c(bonesis_toy_tfs,"JUN","FOS","IRF4","BATF")

dot_plot_input<-as.data.frame(t(raw_input[,which(colnames(raw_input) %in% bonesis_toy_tfs)]))
...
pdf(paste0(analysis_directory,'/figures/dot_plot_bonesis.pdf'),
     width=15,
     height=7)
DotPlot(dot_plot_seurat, features = all.genes,cols = c("blue", "red"), dot.scale = 20) + RotatedAxis()+ ggplot2::scale_colour_gradient2(high = "#ff0000", mid = "#FFFFFF", low = "#0000FF")
dev.off()
...
#Assign the importance to paper interactions
...
#Create a Seurat object ot create scatter plots
pos<-which(bonesis_toy_RF_interactions$target=="JUN-FOS")
bonesis_toy_RF_interactions[pos,"target"]<-"JUN"
rownames(bonesis_toy_RF_interactions)<-seq(nrow(bonesis_toy_RF_interactions))
bonesis_toy_RF_interactions[1,"target"]<-"FOS"

bonesis_toy_RF_tfs<-unique(c(bonesis_toy_RF_interactions$TF,bonesis_toy_RF_interactions$target))
cor_importance<-as.data.frame(t(raw_input))

cell_code<-seq(length(cell_annotations$TYPE))
corr_code<-c()
for(x in seq_along(cell_code)){
  corr_code[x]<-paste0(cell_annotations$TYPE[x],"_",cell_code[x])
}
colnames(cor_importance)<-corr_code

cor_importance_seurat <- CreateSeuratObject(counts = cor_importance, project = "cell_exh", min.cells = 0, min.features = 0)
cor_importance_seurat<-NormalizeData(cor_importance_seurat, normalization.method = "LogNormalize", scale.factor = 10000)
all.genes <- rownames(cor_importance_seurat)
cor_importance_seurat <- ScaleData(cor_importance_seurat, features = all.genes)

cor_importance_plots<-list()
for (x in seq(nrow(bonesis_toy_RF_interactions))){
  cor_importance_plots[[x]]<-FeatureScatter(object = cor_importance_seurat, feature1 = bonesis_toy_RF_interactions$TF[x], feature2 = bonesis_toy_RF_interactions$target[x])
}

cor_importance_plots

FeatureScatter(object = cor_importance_seurat, feature1 = bonesis_toy_RF_interactions$TF[x], feature2 = bonesis_toy_RF_interactions$target[x])
...
for (x in seq(nrow(bonesis_toy_RF_interactions))){
  ...
  bonesis_toy_RF_interactions[x,"Importance"]<-cor(scale(tf),scale(target),method="pearson")
}

cor(input[,which(colnames(input)==bonesis_toy_RF_interactions$target[19])],input[,which(colnames(input)==bonesis_toy_RF_interactions$TF[19])],method="pearson")



#Prepare the dataframe for the Bonesis
for (x in seq_along(bonesis_toy_interactions$Importance)){
  if(bonesis_toy_interactions$Importance[x]=="inhibits"){
    bonesis_toy_interactions[x,"Importance"]<-1*(-1)
  }else{bonesis_toy_interactions[x,"Importance"]<-1}
...
write.table(bonesis_toy_python, file=paste0(analysis_directory,'/bonesis_toy+scenic_python.tsv'), quote=FALSE, sep='\t', row.names = F)
...
DotPlot(dot_plot_seurat, features = all.genes,cols = c("blue", "red"), dot.scale = 10) + RotatedAxis()+ ggplot2::scale_colour_gradient2(high = "#ff0000", mid = "#FFFFFF", low = "#0000FF")
