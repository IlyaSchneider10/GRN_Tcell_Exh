...
#Only keep the one with A confidence
...
for (x in seq(nrow(dorothea_keep))){
  tf<-which(dorothea_keep$tf[x]==frequency_summary$TF)
  target<-which(dorothea_keep$target[x]==frequency_summary$target)
  pos<-tf %in% target
  if(length(pos)!=0){
...
frequency_dorothea_summary<-frequency_summary[keep,]
frequency_dorothea_summary["dorothea_filter"]<-F
frequency_dorothea_summary[keep,"dorothea_filter"]<-T
```
...
#Bonesis Toy
```{r}
bonesis_toy_nodes<-read.csv(paste0(analysis_directory,"/cytoscape/Output/cytoscape_bolouri_RF_fusion.tsv_Bonesis_toy_nodes.csv"),header=T)[,c(1,2,3)]
bonesis_toy_edges<-read.csv(paste0(analysis_directory,"/cytoscape/Output/cytoscape_bolouri_RF_fusion.tsv_Bonesis_toy_edges.csv"),header=T)[,3]
