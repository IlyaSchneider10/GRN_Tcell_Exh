...
V(g_paper_overlap)$degree <- degree(g_paper_overlap)

# Select vertices with highest degrees
v.degree <- degree(g_paper_overlap)
v.sorted <- sort(v.degree, decreasing = TRUE)
v.sorted<-names(v.sorted)
...
louvain_clustering_paper<-cluster_louvain(as.undirected(g_paper_overlap),weight=frequency_summary_top100_paper$NIS, resolution = 0.2)
...
```{r}
#Remove importance column and rows that dont vontain TFs
bolouri_paper_interactions_overlap<-bolouri_paper_interactions_overlap[,c(1,2)]
bolouri_paper_interactions_overlap<-bolouri_paper_interactions_overlap[-c(14,15,16),]
...
bolouri_paper_unique_interactions<-bolouri_paper_interactions_overlap
...
nodes_to_add_expr<-c("mem","mem","exh","mem","rec","mem","exh","exh","mem","mem","exh","exh","exh","exh","mem","mem","mem","rec","rec","rec","exh","rec","rec","rec","rec","mem")
...
V(g_fusion)$degree <- degree(g_fusion)

# Select vertices with highest degrees
v.degree <- degree(g_fusion)
v.sorted <- sort(v.degree, decreasing = TRUE)
v.sorted<-names(v.sorted)
...
#Louvain clustering
louvain_clustering_fusion<-cluster_louvain(as.undirected(g_fusion),weight=igraph_fusion$NIS, resolution =0.7)
...
#Save the table
setwd(analysis_directory)
write.table(bolouri_RF_fusion, file='cytoscape_bolouri_RF_fusion.tsv', quote=FALSE, sep='\t', row.names = F)
```

#Keep only the nodes that have more than one interaction
```{r}
g_fusion <- graph_from_data_frame(d = igraph_fusion, directed = T, vertices = vertices.df_fusion)
...
v.degree <- degree(g_fusion)
v.sorted <- sort(v.degree, decreasing = TRUE)
remove<-names(which(v.sorted==1))
v.sorted<-names(which(v.sorted>1))

g_fusion<- induced_subgraph(g_fusion, v = v.sorted)
l <- layout_with_fr(g_fusion)

#Color nodes according to their highest expression
colrs <- c("#FB8072","#80B1D3","#EDD94C")
my_color<-colrs[as.numeric(as.factor(V(g_fusion)$most_expressed_in))]

#Add weight to edges
igraph_fusion_more_than_one<-igraph_fusion[-which(igraph_fusion$TF %in% remove),]
igraph_fusion_more_than_one<-igraph_fusion_more_than_one[-which(igraph_fusion_more_than_one$target %in% remove),]

E(g_fusion)$weight<-igraph_fusion_more_than_one$NIS

pdf(paste0(analysis_directory,"/figures/bolouri_RF_fusion_more_than_one_interaction.pdf"),
     width=15,
     height=15)
plot(g_fusion, 
     layout=l,
     ...
     edge.width=E(g_fusion)$weight*3,
     vertex.frame.color='white')
legend(x=-1, y=-0.5, c("Exhausted","Memory","Receptors"), pch=21,
       col="#777777", pt.bg=colrs, pt.cex=2, cex=1.5, bty="n", ncol=1)
dev.off()

#Louvain clustering
louvain_clustering_fusion_more_than_one<-cluster_louvain(as.undirected(g_fusion),weight=igraph_fusion_more_than_one$NIS, resolution =0.50)
louvain_clustering_fusion_more_than_one_communities<-louvain_clustering_fusion_more_than_one$membership
names(louvain_clustering_fusion_more_than_one_communities)<-louvain_clustering_fusion_more_than_one$names
louvain_clustering_fusion_more_than_one_communities<-split(names(louvain_clustering_fusion_more_than_one_communities),louvain_clustering_fusion_more_than_one_communities)

#Modify cytoscape dataframe to export it
vertices.df_fusion_more_then_one<-vertices.df_fusion[vertices.df_fusion$nodes %in% unlist(louvain_clustering_fusion_more_than_one_communities),]

vertices.df_fusion_more_then_one["group"]<-0

for (x in names(louvain_clustering_fusion_more_than_one_communities)){
  for (y in louvain_clustering_fusion_more_than_one_communities[[x]]){
    pos<-which(vertices.df_fusion_more_then_one$nodes==y)
    vertices.df_fusion_more_then_one[pos,"group"]<-x
  }
}

bolouri_RF_fusion_more_than_one<-bolouri_RF_fusion[-which(bolouri_RF_fusion$TF %in% remove),]
bolouri_RF_fusion_more_than_one<-bolouri_RF_fusion_more_than_one[-which(bolouri_RF_fusion_more_than_one$target %in% remove),]

#Add the group informatoon for all the nodes
for (x in seq(nrow(bolouri_RF_fusion_more_than_one))){
  pos<-which(vertices.df_fusion_more_then_one$nodes==bolouri_RF_fusion_more_than_one$TF[x])
  bolouri_RF_fusion_more_than_one[x,"TF_group"]<-vertices.df_fusion_more_then_one[pos,"group"]
  
  pos2<-which(vertices.df_fusion_more_then_one$nodes==bolouri_RF_fusion_more_than_one$target[x])
  bolouri_RF_fusion_more_than_one[x,"target_group"]<-vertices.df_fusion_more_then_one[pos2,"group"]
}

#Save the table
setwd(analysis_directory)
write.table(bolouri_RF_fusion_more_than_one, file='cytoscape_bolouri_RF_fusion_more_than_one.tsv', quote=FALSE, sep='\t', row.names = F)
```
