...
cutoff_thresholds<-c(0.15,-0.06,0.05)
names(cutoff_thresholds)<-names(aucell_scores_norm_cell_types_means)
...
vertices.df <- data.frame(unique(c(frequency_summary_top100$TF,frequency_summary_top100$target)))
...
plot(g.subset, 
     layout=l,
     edge.arrow.size=0.1,
     vertex.size=(3.5*V(g.subset)$degree/max(V(g.subset)$degree)*3.5),
     vertex.label.cex=(V(g.subset)$degree/max(V(g.subset)$degree)*1.5),
     edge.width=0.5,
     vertex.frame.color='white')
dev.off()
